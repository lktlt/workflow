...
import { UrlConfigService } from 'src/app/core/configs/url-config.service';
...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit(): void {
    const connection = new signalR.HubConnectionBuilder()
      .withUrl(`${this.urlConfig._baseUri}/appState`)
      .configureLogging(signalR.LogLevel.Warning)
      .build();

      connection.on("HandleCpuUsage", (d:any) => {
        if (this.cpuChart) {
      ...
      connection.on("HandleWorkingSet", (data: number) => {
        ...
        if (this.memoryChart) {
          ...
          // this.memoryChart.source(this.memoryChartData);
          this.memoryChart.render();
        }
      });
      connection.on("HandelGCCount", (gen: number, count: number) => {
        switch (gen) {
          case 0:
      ...
      connection.on("HandelGcSize", (gen: number, count: number) => {
        switch (gen) {
          case 0:
            this.gen0Size = count;
    ...
    connection.on("HandelThreadPoolThreadCount", (data: number) => {
      this.handelThreadPoolThreadCount = data;
    });

    connection.on("HandelMonitorLockContentionCount", (data: number) => {
      this.handelMonitorLockContentionCount = data;
    });

    connection.on("ThreadPoolQueueLength", (data: number) => {
      this.threadPoolQueueLength = data;
    });

    connection.on("ThreadPoolCompletedWorkItemCount", (data: number) => {
      this.threadPoolCompletedWorkItemCount = data;
    });

    connection.on("HandleGCHeapSize", (data: number) => {
      this.gcHeapSize = data;
    });
    connection.on("HandeLohSize", (data: number) => {
      this.lohSize = data;
    });
    connection.on("HandelPohSize", (data: number) => {
      this.pohSize = data;
    });
  ...
  drawCpuChart(chart: Chart, data: {value:number}[]) {
    const color = ['#0086FA', '#FFBF00', '#F5222D'];
